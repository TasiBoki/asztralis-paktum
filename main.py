...
class WishRequest(BaseModel):
    ...
    def validate_phone(cls, v: str) -> str:
        ...
        if not re.match(pattern, cleaned):
            raise ValueError(
                "Érvénytelen telefonszám formátum! Használd a +36301234567 vagy 06301234567 formátumot."
            )
        return cleaned
...
def check_if_banned(username: str, phone: str, session: Session) -> bool:
    statement = select(User).where(
        User.username == username, User.phone == phone, User.status == "rejected"
    )
    rejected_wishes = session.exec(statement).all()
    return len(rejected_wishes) >= 3


def validate_and_update_phone_active(
    phone: str, username: str, session: Session
):
    cleaned_phone = re.sub(r"[\s\-()]", "", phone)
    statement_phone_active = select(User).where(
        User.phone == cleaned_phone, User.status == "active"
    ...
    if active_on_phone:
        if datetime.now() < active_on_phone.expires_at:
            if active_on_phone.username != username:
                raise HTTPException(
                    status_code=400,
                    detail=f"Erről a telefonszámról már fut egy aktív paktum '{active_on_phone.username}' néven! Várj, amíg lejár.",
                )
            else:
                raise HTTPException(
                    status_code=400,
                    detail="Még fut egy aktív paktumod ezen a készüléken!",
                )
        else:
...
def read_admin():
    if os.path.exists("admin.html"):
        ...
        return "<h3>Az admin.html fájl nem található a gyökérben!</h3>"
...
def submit_wish(data: WishRequest, session: Session = Depends(get_session)):
    ...
    if check_if_banned(data.username, cleaned_phone, session):
        raise HTTPException(
            status_code=403,
            detail="Ezt az azonosítót és telefonszámot véglegesen letiltotta az Univerzum.",
        )
    ...
    return {"message": "A paktum létrejött.", "expires_at": expiry_time.isoformat()}
...
async def submit_wish_video(
    ...
    if not re.match(pattern, cleaned_phone):
        raise HTTPException(
            status_code=400, detail="Érvénytelen telefonszám formátum!"
        )
    ...
    try:
        file_extension = (
            video.filename.split(".")[-1] if "." in video.filename else "webm"
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Hiba történt a videó feldolgozása közben: {str(e)}",
        )
...
def get_active_wishes(session: Session = Depends(get_session)):
    statement = (
        select(User).where(User.status == "active").order_by(User.id.asc())
    )
...
def admin_archive_evidence(wish_id: int, session: Session = Depends(get_session)):
    ...
    return {
        "message": "Bizonyíték sikeresen archiválva a hatóságok számára!",
        "data": archived_info,
    }
...
def reject_wish(
    wish_id: int, data: RejectRequest, session: Session = Depends(get_session)
):
    user = session.get(User, wish_id)
    if not user:
...
def admin_ban_user(wish_id: int, session: Session = Depends(get_session)):
    ...
    session.commit()
    return {
        "message": f"A(z) {user.username} ({user.phone}) páros véglegesen letiltva."
    }


@app.get("/check-user/{username}/{phone}")
def check_user_status(
    username: str, phone: str, session: Session = Depends(get_session)
):
    cleaned_phone = re.sub(r"[\s\-()]", "", phone)
    ...
    statement = select(User).where(
        User.username == username, User.phone == cleaned_phone
    ).order_by(User.id.desc())
    users = session.exec(statement).all()
